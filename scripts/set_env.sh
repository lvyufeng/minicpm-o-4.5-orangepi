...
#
# Override either path with the env vars MINICPMO_ASCEND_TOOLKIT_ROOT or
# MINICPMO_CUSTOM_OPP_VENDOR if your install lives elsewhere.

set -euo pipefail

REPO_ROOT=$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)

export ASCEND_TOOLKIT_ROOT="${MINICPMO_ASCEND_TOOLKIT_ROOT:-/usr/local/Ascend/ascend-toolkit/latest/aarch64-linux}"
export LD_LIBRARY_PATH="$ASCEND_TOOLKIT_ROOT/lib64:${LD_LIBRARY_PATH:-}"

CUSTOM_OPP_VENDOR="${MINICPMO_CUSTOM_OPP_VENDOR:-$REPO_ROOT/custom_opp_install/vendors/customize}"
