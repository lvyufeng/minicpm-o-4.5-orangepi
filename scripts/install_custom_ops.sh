...
if [[ -z "${ASCEND_HOME_PATH:-}" && -z "${ASCEND_AICPU_PATH:-}" && -z "${BASE_LIBS_PATH:-}" ]]; then
    CANN_ENV="${MINICPMO_ASCEND_TOOLKIT_ROOT:-/usr/local/Ascend/ascend-toolkit/latest}/bin/setenv.bash"
    if [[ -f "$CANN_ENV" ]]; then
        # shellcheck disable=SC1090
        source "$CANN_ENV"
    else
        export ASCEND_HOME_PATH="${MINICPMO_ASCEND_TOOLKIT_ROOT:-/usr/local/Ascend/ascend-toolkit/latest}"
    fi
fi
