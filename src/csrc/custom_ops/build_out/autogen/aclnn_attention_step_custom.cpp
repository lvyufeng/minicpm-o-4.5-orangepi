...
[[maybe_unused]] uint32_t NNOPBASE_AttentionStepCustom = 0U;
} // namespace
    ...
    aclOpExecutor **executor)
{
    ...
    static NnopbaseDfxId tilingId = {0x60000, "aclnnAttentionStepCustomTiling", false};
    void *nnopExecutor;
    static void *executorSpace = NULL;
    const char *opType = "AttentionStepCustom";
    char inputDesc[] = {1, 1, 1};
    char outputDesc[] = {1};
    char attrDesc[] = {1, 1, 1, 1};

    NNOPBASE_ASSERT_NOTNULL_RETVAL(query);
    NNOPBASE_ASSERT_NOTNULL_RETVAL(kCache);
    NNOPBASE_ASSERT_NOTNULL_RETVAL(vCache);
    ...
    *executor = reinterpret_cast<aclOpExecutor *>(nnopExecutor);
    ...
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, kCache, 1));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, vCache, 2));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddAttrWithDtype(*executor, static_cast<void*>(&context), sizeof(int64_t), 0, kNnopbaseInt));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddAttrWithDtype(*executor, static_cast<void*>(&numQHeads), sizeof(int64_t), 1, kNnopbaseInt));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddAttrWithDtype(*executor, static_cast<void*>(&numKvHeads), sizeof(int64_t), 2, kNnopbaseInt));
    float tmp3 = static_cast<float>(scale);
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddAttrWithDtype(*executor, static_cast<void*>(&tmp3), sizeof(float), 3, kNnopbaseFloat));
    ...
    if (NnopbaseAddParamName != NULL) {
        ...
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddParamName(*executor, 1, "kCache", true));
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddParamName(*executor, 2, "vCache", true));
