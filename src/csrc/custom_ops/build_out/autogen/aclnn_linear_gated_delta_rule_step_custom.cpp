...
TensorDesc outputDesc0_0[1] =
    {{ge::DT_FLOAT16, ge::FORMAT_ND}};
SupportInfo list0_0 = {inputDesc0_0, 5, outputDesc0_0, 1};
...
[[maybe_unused]] uint32_t NNOPBASE_LinearGatedDeltaRuleStepCustom = 0U;
} // namespace
    ...
    aclOpExecutor **executor)
{
    ...
    static NnopbaseDfxId tilingId = {0x60000, "aclnnLinearGatedDeltaRuleStepCustomTiling", false};
    void *nnopExecutor;
    static void *executorSpace = NULL;
    const char *opType = "LinearGatedDeltaRuleStepCustom";
    char inputDesc[] = {1, 1, 1, 1, 1};
    char outputDesc[] = {1};
    char attrDesc[] = {};

    NNOPBASE_ASSERT_NOTNULL_RETVAL(mixed);
    NNOPBASE_ASSERT_NOTNULL_RETVAL(beta);
    NNOPBASE_ASSERT_NOTNULL_RETVAL(decay);
    NNOPBASE_ASSERT_NOTNULL_RETVAL(state);
    NNOPBASE_ASSERT_NOTNULL_RETVAL(scratch);
    ...
    *executor = reinterpret_cast<aclOpExecutor *>(nnopExecutor);
    ...
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, mixed, 0));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, beta, 1));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, decay, 2));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, state, 3));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, scratch, 4));
    ...
    if (NnopbaseAddParamName != NULL) {
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddParamName(*executor, 0, "mixed", true));
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddParamName(*executor, 1, "beta", true));
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddParamName(*executor, 2, "decay", true));
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddParamName(*executor, 3, "state", true));
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddParamName(*executor, 4, "scratch", true));
