...
[[maybe_unused]] uint32_t NNOPBASE_MatmulCubeCustom = 0U;
} // namespace
    ...
    aclOpExecutor **executor)
{
    ...
    static NnopbaseDfxId tilingId = {0x60000, "aclnnMatmulCubeCustomTiling", false};
    void *nnopExecutor;
    static void *executorSpace = NULL;
    const char *opType = "MatmulCubeCustom";
    char inputDesc[] = {1, 1};
    char outputDesc[] = {1};
    char attrDesc[] = {};

    NNOPBASE_ASSERT_NOTNULL_RETVAL(a);
    NNOPBASE_ASSERT_NOTNULL_RETVAL(b);
    ...
    *executor = reinterpret_cast<aclOpExecutor *>(nnopExecutor);
    ...
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, a, 0));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, b, 1));
    ...
    if (NnopbaseAddParamName != NULL) {
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddParamName(*executor, 0, "a", true));
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddParamName(*executor, 1, "b", true));
