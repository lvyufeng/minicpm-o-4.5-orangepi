...
[[maybe_unused]] uint32_t NNOPBASE_LinearCausalConvStepCustom = 0U;
} // namespace
    ...
    aclOpExecutor **executor)
{
    ...
    static NnopbaseDfxId tilingId = {0x60000, "aclnnLinearCausalConvStepCustomTiling", false};
    void *nnopExecutor;
    static void *executorSpace = NULL;
    const char *opType = "LinearCausalConvStepCustom";
    char inputDesc[] = {1, 1};
    char outputDesc[] = {1};
    char attrDesc[] = {};

    NNOPBASE_ASSERT_NOTNULL_RETVAL(x);
    NNOPBASE_ASSERT_NOTNULL_RETVAL(weightT);
    ...
    *executor = reinterpret_cast<aclOpExecutor *>(nnopExecutor);
    ...
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, weightT, 1));
    ...
    if (NnopbaseAddParamName != NULL) {
        ...
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddParamName(*executor, 1, "weightT", true));
