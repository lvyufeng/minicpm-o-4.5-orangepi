...
[[maybe_unused]] uint32_t NNOPBASE_W8a8DequantCustom = 0U;
} // namespace
    ...
    aclOpExecutor **executor)
{
    ...
    static NnopbaseDfxId tilingId = {0x60000, "aclnnW8a8DequantCustomTiling", false};
    void *nnopExecutor;
    static void *executorSpace = NULL;
    const char *opType = "W8a8DequantCustom";
    char inputDesc[] = {1, 1, 1};
    char outputDesc[] = {1};
    char attrDesc[] = {};

    NNOPBASE_ASSERT_NOTNULL_RETVAL(acc);
    NNOPBASE_ASSERT_NOTNULL_RETVAL(xScale);
    NNOPBASE_ASSERT_NOTNULL_RETVAL(wScale);
    ...
    *executor = reinterpret_cast<aclOpExecutor *>(nnopExecutor);
    ...
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, acc, 0));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, xScale, 1));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, wScale, 2));
    ...
    if (NnopbaseAddParamName != NULL) {
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddParamName(*executor, 0, "acc", true));
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddParamName(*executor, 1, "xScale", true));
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddParamName(*executor, 2, "wScale", true));
