...
[[maybe_unused]] uint32_t NNOPBASE_SiluMulCustom = 0U;
} // namespace
...
aclnnStatus aclnnSiluMulCustomGetWorkspaceSize(
    ...
    aclOpExecutor **executor)
{
    ...
    static NnopbaseDfxId tilingId = {0x60000, "aclnnSiluMulCustomTiling", false};
    void *nnopExecutor;
    static void *executorSpace = NULL;
    const char *opType = "SiluMulCustom";
    char inputDesc[] = {1, 1};
    char outputDesc[] = {1};
    char attrDesc[] = {};

    NNOPBASE_ASSERT_NOTNULL_RETVAL(gate);
    NNOPBASE_ASSERT_NOTNULL_RETVAL(up);
    ...
    *executor = reinterpret_cast<aclOpExecutor *>(nnopExecutor);
    ...
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, gate, 0));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, up, 1));
    ...
    if (NnopbaseAddParamName != NULL) {
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddParamName(*executor, 0, "gate", true));
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddParamName(*executor, 1, "up", true));
...
aclnnStatus aclnnSiluMulCustom(
    void *workspace,
    uint64_t workspaceSize,
    aclOpExecutor *executor,
    aclrtStream stream)
