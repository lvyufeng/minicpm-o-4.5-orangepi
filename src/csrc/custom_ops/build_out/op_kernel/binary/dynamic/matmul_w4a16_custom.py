...
def _build_args(x_in__, w_in__, scales_in__, out_out_):
    __inputs__ = []
    for arg in [x_in__, w_in__, scales_in__]:
        if arg != None:
...
@tbe_register.register_operator("MatmulW4a16Custom", trans_bool_to_s8=False)
...
def matmul_w4a16_custom(x_in__, w_in__, scales_in__, out_out_, kernel_name="matmul_w4a16_custom", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, w_in__, scales_in__, out_out_)
    ...
    origin_func_name = "matmul_w4a16_custom"
    ascendc_src_dir_ex = "matmul_w4a16_custom"
    ascendc_src_dir = "matmul_w4a16_custom"
    ascendc_src_file = "matmul_w4a16_custom.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Ascend C Operator MatmulW4a16Custom, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "MatmulW4a16Custom"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__ , impl_mode = impl_mode, origin_inputs=[x_in__, w_in__, scales_in__], origin_outputs = [out_out_],\
...
def op_select_format(x_in__, w_in__, scales_in__, out_out_, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, w_in__, scales_in__, out_out_)
    result = check_op_cap("op_select_format", "MatmulW4a16Custom", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, w_in__, scales_in__, out_out_, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, w_in__, scales_in__, out_out_)
    result = check_op_cap("get_op_specific_info", "MatmulW4a16Custom", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
