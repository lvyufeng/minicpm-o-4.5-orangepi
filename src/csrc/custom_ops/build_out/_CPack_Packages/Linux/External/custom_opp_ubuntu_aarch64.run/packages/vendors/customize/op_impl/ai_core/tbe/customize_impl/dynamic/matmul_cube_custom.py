...
def _build_args(a_in__, b_in__, c_out_):
    __inputs__ = []
    for arg in [a_in__, b_in__]:
        if arg != None:
...
@tbe_register.register_operator("MatmulCubeCustom", trans_bool_to_s8=False)
...
def matmul_cube_custom(a_in__, b_in__, c_out_, kernel_name="matmul_cube_custom", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(a_in__, b_in__, c_out_)
    ...
    origin_func_name = "matmul_cube_custom"
    ascendc_src_dir_ex = "matmul_cube_custom"
    ascendc_src_dir = "matmul_cube_custom"
    ascendc_src_file = "matmul_cube_custom.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Ascend C Operator MatmulCubeCustom, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "MatmulCubeCustom"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__ , impl_mode = impl_mode, origin_inputs=[a_in__, b_in__], origin_outputs = [c_out_],\
...
def op_select_format(a_in__, b_in__, c_out_, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(a_in__, b_in__, c_out_)
    result = check_op_cap("op_select_format", "MatmulCubeCustom", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(a_in__, b_in__, c_out_, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(a_in__, b_in__, c_out_)
    result = check_op_cap("get_op_specific_info", "MatmulCubeCustom", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
