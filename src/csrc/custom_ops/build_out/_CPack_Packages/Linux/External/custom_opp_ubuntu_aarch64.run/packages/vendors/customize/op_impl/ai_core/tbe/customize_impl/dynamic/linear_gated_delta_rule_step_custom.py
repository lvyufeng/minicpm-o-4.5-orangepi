...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['mixed', 'beta', 'decay', 'state', 'scratch']
    output_names = ['out']
...
def _build_args(mixed_in__, beta_in__, decay_in__, state_in__, scratch_in__, out_out_):
    __inputs__ = []
    for arg in [mixed_in__, beta_in__, decay_in__, state_in__, scratch_in__]:
        if arg != None:
...
@tbe_register.register_operator("LinearGatedDeltaRuleStepCustom", trans_bool_to_s8=False)
...
def linear_gated_delta_rule_step_custom(mixed_in__, beta_in__, decay_in__, state_in__, scratch_in__, out_out_, kernel_name="linear_gated_delta_rule_step_custom", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(mixed_in__, beta_in__, decay_in__, state_in__, scratch_in__, out_out_)
    ...
    origin_func_name = "linear_gated_delta_rule_step_custom"
    ascendc_src_dir_ex = "linear_gated_delta_rule_step_custom"
    ascendc_src_dir = "linear_gated_delta_rule_step_custom"
    ascendc_src_file = "linear_gated_delta_rule_step_custom.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Ascend C Operator LinearGatedDeltaRuleStepCustom, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "LinearGatedDeltaRuleStepCustom"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__ , impl_mode = impl_mode, origin_inputs=[mixed_in__, beta_in__, decay_in__, state_in__, scratch_in__], origin_outputs = [out_out_],\
...
def op_select_format(mixed_in__, beta_in__, decay_in__, state_in__, scratch_in__, out_out_, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(mixed_in__, beta_in__, decay_in__, state_in__, scratch_in__, out_out_)
    result = check_op_cap("op_select_format", "LinearGatedDeltaRuleStepCustom", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(mixed_in__, beta_in__, decay_in__, state_in__, scratch_in__, out_out_, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(mixed_in__, beta_in__, decay_in__, state_in__, scratch_in__, out_out_)
    result = check_op_cap("get_op_specific_info", "LinearGatedDeltaRuleStepCustom", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
