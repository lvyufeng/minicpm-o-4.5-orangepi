...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['query', 'k_cache', 'v_cache']
    output_names = ['out']
...
def _build_args(query_in__, k_cache_in__, v_cache_in__, out_out_, context, num_q_heads, num_kv_heads, scale):
    __inputs__ = []
    for arg in [query_in__, k_cache_in__, v_cache_in__]:
        if arg != None:
    ...
    if num_q_heads != None:
        attr = {}
        attr["name"] = "num_q_heads"
        attr["dtype"] = "int"
        attr["value"] = num_q_heads
        __attrs__.append(attr)
...
@tbe_register.register_operator("AttentionStepCustom", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_FLOAT, para_check.KERNEL_NAME)
def attention_step_custom(query_in__, k_cache_in__, v_cache_in__, out_out_, context, num_q_heads, num_kv_heads, scale, kernel_name="attention_step_custom", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(query_in__, k_cache_in__, v_cache_in__, out_out_, context, num_q_heads, num_kv_heads, scale)
    ...
    origin_func_name = "attention_step_custom"
    ascendc_src_dir_ex = "attention_step_custom"
    ascendc_src_dir = "attention_step_custom"
    ascendc_src_file = "attention_step_custom.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Ascend C Operator AttentionStepCustom, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "AttentionStepCustom"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__ , impl_mode = impl_mode, origin_inputs=[query_in__, k_cache_in__, v_cache_in__], origin_outputs = [out_out_],\
...
def op_select_format(query_in__, k_cache_in__, v_cache_in__, out_out_, context, num_q_heads, num_kv_heads, scale, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(query_in__, k_cache_in__, v_cache_in__, out_out_, context, num_q_heads, num_kv_heads, scale)
    result = check_op_cap("op_select_format", "AttentionStepCustom", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(query_in__, k_cache_in__, v_cache_in__, out_out_, context, num_q_heads, num_kv_heads, scale, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(query_in__, k_cache_in__, v_cache_in__, out_out_, context, num_q_heads, num_kv_heads, scale)
    result = check_op_cap("get_op_specific_info", "AttentionStepCustom", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
