...
@tbe_register.register_operator("MatmulVecCustom", trans_bool_to_s8=False)
...
def matmul_vec_custom(x_in__, w_in__, out_out_, kernel_name="matmul_vec_custom", impl_mode = ""):
    ...
    origin_func_name = "matmul_vec_custom"
    ascendc_src_dir_ex = "matmul_vec_custom"
    ascendc_src_dir = "matmul_vec_custom"
    ascendc_src_file = "matmul_vec_custom.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Ascend C Operator MatmulVecCustom, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "MatmulVecCustom"
...
def op_select_format(x_in__, w_in__, out_out_, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, w_in__, out_out_)
    result = check_op_cap("op_select_format", "MatmulVecCustom", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, w_in__, out_out_, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, w_in__, out_out_)
    result = check_op_cap("get_op_specific_info", "MatmulVecCustom", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
