...
def _build_args(gate_in__, up_in__, out_out_):
    __inputs__ = []
    for arg in [gate_in__, up_in__]:
        if arg != None:
...
@tbe_register.register_operator("SiluMulCustom", trans_bool_to_s8=False)
...
def silu_mul_custom(gate_in__, up_in__, out_out_, kernel_name="silu_mul_custom", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(gate_in__, up_in__, out_out_)
    ...
    origin_func_name = "silu_mul_custom"
    ascendc_src_dir_ex = "silu_mul_custom"
    ascendc_src_dir = "silu_mul_custom"
    ascendc_src_file = "silu_mul_custom.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Ascend C Operator SiluMulCustom, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "SiluMulCustom"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__ , impl_mode = impl_mode, origin_inputs=[gate_in__, up_in__], origin_outputs = [out_out_],\
...
def op_select_format(gate_in__, up_in__, out_out_, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(gate_in__, up_in__, out_out_)
    result = check_op_cap("op_select_format", "SiluMulCustom", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(gate_in__, up_in__, out_out_, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(gate_in__, up_in__, out_out_)
    result = check_op_cap("get_op_specific_info", "SiluMulCustom", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
