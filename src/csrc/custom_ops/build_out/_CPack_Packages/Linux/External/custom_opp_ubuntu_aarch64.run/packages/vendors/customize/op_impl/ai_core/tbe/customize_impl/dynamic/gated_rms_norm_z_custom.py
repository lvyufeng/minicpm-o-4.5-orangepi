...
def _build_args(core_in__, z_silu_in__, gamma_in__, out_out_):
    __inputs__ = []
    for arg in [core_in__, z_silu_in__, gamma_in__]:
        if arg != None:
...
@tbe_register.register_operator("GatedRmsNormZCustom", trans_bool_to_s8=False)
...
def gated_rms_norm_z_custom(core_in__, z_silu_in__, gamma_in__, out_out_, kernel_name="gated_rms_norm_z_custom", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(core_in__, z_silu_in__, gamma_in__, out_out_)
    ...
    origin_func_name = "gated_rms_norm_z_custom"
    ascendc_src_dir_ex = "gated_rms_norm_z_custom"
    ascendc_src_dir = "gated_rms_norm_z_custom"
    ascendc_src_file = "gated_rms_norm_z_custom.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Ascend C Operator GatedRmsNormZCustom, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "GatedRmsNormZCustom"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__ , impl_mode = impl_mode, origin_inputs=[core_in__, z_silu_in__, gamma_in__], origin_outputs = [out_out_],\
...
def op_select_format(core_in__, z_silu_in__, gamma_in__, out_out_, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(core_in__, z_silu_in__, gamma_in__, out_out_)
    result = check_op_cap("op_select_format", "GatedRmsNormZCustom", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(core_in__, z_silu_in__, gamma_in__, out_out_, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(core_in__, z_silu_in__, gamma_in__, out_out_)
    result = check_op_cap("get_op_specific_info", "GatedRmsNormZCustom", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
